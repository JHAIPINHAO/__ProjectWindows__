...
class Window(tk.Tk):
    def __init__(self,**kwargs):
        ...
        Button(self,text="台灣目前aqi最差的3個站點",font=('Helvetica', '24'),pady=10,command=self.btn2_click).pack(fill=tk.X)
    ...
    def get_bad(self,dataList) -> list:
        sorted_data = sorted(dataList,key=lambda a:a.aqi,reverse=True)
        def out_aqi_999(site):
            return site.aqi != 999
        filter_data = filter(out_aqi_999,sorted_data)
    ...
    def btn2_click(self):
        bad3_list = self.get_bad(self.aqi_list)
